import { Layers, X, ChevronLeft, ChevronRight, ImageIcon, ExternalLink, Plus, Trash2 } from "lucide-react";
  ...
  return (
    <aside className="absolute right-0 top-0 h-full w-[380px] bg-zinc-950 border-l border-zinc-900 flex flex-col z-50 shadow-2xl">
        <div className="p-5 border-b border-zinc-900 flex justify-between items-center bg-orange-600">
            <div className="flex items-center gap-2 text-white"><Layers size={18}/><span className="text-sm font-bold uppercase">{data.label || 'Carrossel Ad'}</span></div>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-8">
            ...
            <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-900 space-y-5">
                <div className="flex justify-between items-center pb-2 border-b border-zinc-800"><Label className="text-xs text-blue-500 font-bold uppercase">Slides ({currentSlide+1}/{slides.length})</Label></div>
                ...
                <div className="space-y-3">
                    ...
                    <Textarea placeholder="Texto do Slide (P)" value={slides[currentSlide].p} onChange={(e)=>updateSlide('p', e.target.value)} className="bg-black border-zinc-800 text-white min-h-[70px] text-xs" onFocus={(e)=>e.target.select()}/>
                </div>
                <div className="p-3 rounded-lg border bg-red-900/10 border-red-900/30 space-y-3">
                    <div className="flex gap-3">
                        <div className="flex-1 space-y-1"><Label className="text-[10px] text-zinc-500 uppercase font-bold">CTA (Botão)</Label><Input placeholder="Saiba Mais" className="bg-black border-zinc-800 text-white text-xs h-9" value={slides[currentSlide].cta} onChange={(e)=>updateSlide('cta', e.target.value)} onFocus={(e)=>e.target.select()}/></div>
                        <div className="w-1/3 space-y-1"><Label className="text-[10px] text-zinc-500 uppercase font-bold">Freq. Máx</Label><Input value={slides.length} readOnly className="bg-black border-zinc-800 text-white text-xs h-9 text-center cursor-not-allowed"/></div>
                    </div>
                    <div className="relative"><ExternalLink size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500"/><Input placeholder="URL Externa (https://)" className="bg-black border-zinc-800 text-white text-xs h-9 pl-8" value={slides[currentSlide].url} onChange={(e)=>updateSlide('url', e.target.value)} onFocus={(e)=>e.target.select()}/></div>
                </div>
                ...
                <div className="space-y-1 pt-3 border-t border-zinc-800"><Label className="text-[10px] text-zinc-500 uppercase font-bold">Freq. Máx Global</Label><Input type="number" placeholder="3" value={data.frequency || 3} onChange={(e) => updateField('frequency', e.target.value)} className="bg-black border-zinc-800 text-white text-center font-bold h-9" /></div>
            </div>
        </div>
