...
export default function QuizAdPanel({ data, nodeId, onChange, onClose }: any) {
  ...
  const addQuestion = () => { const q = data.questions || []; if(q.length < 5) updateField('questions', [...q, { id: Date.now(), text: '', mandatory: false, options: ['Opção A'] }]); };
  ...
  return (
    <aside className="absolute right-0 top-0 h-full w-[380px] bg-zinc-950 border-l border-zinc-900 flex flex-col z-50 shadow-2xl">
        <div className="p-5 border-b border-zinc-900 flex justify-between items-center bg-indigo-600">
            <div className="flex items-center gap-2 text-white"><FileText size={18}/><span className="text-sm font-bold uppercase">{data.label || 'Quiz Ad'}</span></div>
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-8">
            <div className="space-y-6">
                ...
                <Input value={data.label} onChange={(e) => updateField('label', e.target.value)} className="bg-black border-zinc-800 text-white h-10" placeholder="Nome do Quiz" onFocus={(e)=>e.target.select()}/>
                <div className="grid grid-cols-2 gap-3 h-32">
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex-1 border border-dashed border-zinc-700 rounded-lg flex items-center justify-center relative"><span className="text-[9px] text-zinc-600">Logo</span><input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e)=>handleUpload(e,'logo')}/></div>
                        <div className="flex-1 border border-dashed border-zinc-700 rounded-lg flex items-center justify-center relative"><span className="text-[9px] text-zinc-600">Bg</span><input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e)=>handleUpload(e,'background')}/></div>
                    </div>
                </div>
                ...
                <div className="space-y-5">
                    ...
                    {(data.questions || []).map((q: any, qIdx: number) => (
                        <div key={q.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 space-y-4 relative">
                            ...
                            <div className="flex flex-col items-end gap-1"><span className="text-[9px] text-zinc-500 font-bold uppercase">Obrigatória?</span><Switch className="scale-75 data-[state=checked]:bg-green-500" checked={q.mandatory} onCheckedChange={(v)=>updateQuestion(qIdx, 'mandatory', v)}/></div>
                        </div>
                    ))}
                </div>
            </div>
