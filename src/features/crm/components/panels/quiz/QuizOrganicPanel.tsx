...
export default function QuizOrganicPanel({ data, nodeId, onChange, onClose }: any) {
  ...
  const switchClass = "data-[state=checked]:bg-indigo-600"; 
  ...
  const handleAddQuestion = () => {
    ...
    onChange(nodeId, { 
        questions: [...questions, { id: Date.now(), text: '', type: 'single', options: [{ id: 1, text: '' }, { id: 2, text: '' }] }] 
    });
  };
  ...
  const addOption = (qIndex: number) => {
      const newQ = [...questions];
      newQ[qIndex].options.push({ id: Date.now(), text: '' });
      onChange(nodeId, { questions: newQ });
  };
    ...
    <div className="flex flex-col h-full w-full bg-zinc-950 text-zinc-100 selection:bg-indigo-600 selection:text-white">
      ...
      <div className={`flex items-center justify-between px-4 py-3 ${headerColor} shrink-0`}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-white uppercase tracking-wider">Configuração de Pesquisa</span>
        </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 mt-2">
              ...
              <p className="text-[8px] text-zinc-500 italic">*Se deixados em branco, apenas a imagem de capa será exibida.</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
            <Label className={`text-[10px] font-bold uppercase tracking-wider ${textColor}`}>SMART LINK INTERNO (CTA)</Label>
            <Input 
                value={data.url || ''} 
                ...
                placeholder="socialone://post/internal-link" 
            />
        </div>
